...
'''
import sys
sys.setrecursionlimit(10**9) #setrecursionlimit이란 재귀 가능횟수를 제한하는 것 / sys.maxsize는 아주 큰 정수값이
def perm(arr, end, cur, remainder): #arr 찾을 배열, end 몇개를 찾을지, cur 현재 글자, remainder 조합 가능한 남은 글자
    ...
    if end == cur: #현재 글자 크기가 end에 도달하면 종료
        ...
        for next in remainder:
            ret+=(perm(end, cur+1, val+next, remainder - set(next)))
    return ret
'''
...
def perm(arr, end): #arr는 찾을 배열, end는 몇개를 찾을지
    ...
    def func(end, cur, remainder):
        ...
        if end == len(cur):#현재 글자 크기가 end에 도달하면 종료
            ...
            for next in remainder:
                ret+=(func(end, cur+next, remainder - set(next)))
        return ret
    return func(end, "", set(arr))
'''
모든 개수에 대해 순열을 구하려면 end에 1,2,3..을 대입하여 모두 구해도되지만
실시간을 아끼기 위해서는 다음의 함수를 사용해도된다.
기본적인 알고리즘은 비슷하다.
...
def combiAll_FromArrs(arrs):
    def func(end, cur, idx):
        ret = list()
        if end == idx:
            ret.append(cur)
        else:
            for next in arrs[idx]:
                ret += func(end, cur+next, idx+1)
        return ret
    return func(len(arrs), "", 0)
'''
